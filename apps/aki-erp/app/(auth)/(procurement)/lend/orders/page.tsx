...
import { deleteLendOrderId, fetchLendOrder, patchArtworksBatchId } from '@data-access/apis';
...
const LendOrders = () => {
  ...
  const columns: ColumnDef<LendOrder, any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Link
          ...
          href={`/lend/orders/${row.original.id}?${searchParams.toString()}`}
        >
    ...
    },
    {
      ...
      cell: ({ cell }: CellContext<LendOrder, ArtworkDetail['metadata']>) =>
    ...
    },
    {
      ...
      header: '銷售狀態',
      accessorKey: 'artworks.0.metadata',
      cell: ({ getValue }: CellContext<LendOrder, ArtworkDetail['metadata']>) => {
  ...
  const deleteMutation = useMutation(
    (LendOrders: LendOrder[]) => {
      const artworkIds = LendOrders.flatMap((item) => item.artworks?.map((item) => item.id));
      return Promise.all([
        ...LendOrders.map((item) => deleteLendOrderId(item.id)),
