...
const TransferOrders = () => {
  ...
  const columns: ColumnDef<TransferOrder, any>[] = [
    {
      ...
      cell: ({ row }) => (
        <Link
          ...
          href={`/transfer/orders/${row.original.id}?${searchParams.toString()}`}
        >
  ...
  const deleteMutation = useMutation(
    (transferOrders: TransferOrder[]) => {
      const artworkIds = transferOrders.flatMap((item) => item.artworks?.map((item) => item.id));
      return Promise.all([
        ...transferOrders.map((item) => deleteTransferOrderId(item.id)),
