...
import { ArtworkDetail, SalesOrder, Status } from 'data-access/models';
...
const ShipmentOrders = () => {
  ...
  const columns: ColumnDef<SalesOrder, any>[] = [
    ...
    },
    {
      ...
      cell: ({ cell }: CellContext<SalesOrder, ArtworkDetail['metadata']>) =>
    ...
    },
    {
      ...
      header: '銷售狀態',
      accessorKey: 'artworks.0.metadata',
      cell: ({ getValue }: CellContext<SalesOrder, ArtworkDetail['metadata']>) => {
  ...
  const { table, tableBlock } = useTable<SalesOrder>({
    data,
    columns,
    isLoading,
  });
