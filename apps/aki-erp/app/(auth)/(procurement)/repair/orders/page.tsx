...
import { deleteRepairOrderId, fetchRepairOrder, patchArtworksBatchId } from '@data-access/apis';
...
import { ArtworkDetail, RepairOrder, Status } from 'data-access/models';
...
const RepairOrders = () => {
  ...
  const columns: ColumnDef<RepairOrder, any>[] = [
    ...
    },
    {
      ...
      cell: ({ cell }: CellContext<RepairOrder, ArtworkDetail['metadata']>) =>
    ...
    },
    {
      ...
      header: '銷售狀態',
      accessorKey: 'artworks.0.metadata',
      cell: ({ getValue }: CellContext<RepairOrder, ArtworkDetail['metadata']>) => {
  ...
  const { table, tableBlock } = useTable<RepairOrder>({
    data,
    columns,
    isLoading,
  });
