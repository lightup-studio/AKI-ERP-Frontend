...
import { ArtworkDetail, RepairReturnOrder, Status } from 'data-access/models';
...
const RepairReturnOrders = () => {
  ...
  const { getSelectAllProps, getSelectItemProps, selectedRowCount, selectedRows, clearSelection } =
    useSelectionList<RepairReturnOrder>();
  ...
  const columns: ColumnDef<RepairReturnOrder, any>[] = [
    ...
    },
    {
      ...
      cell: ({ cell }: CellContext<RepairReturnOrder, ArtworkDetail['metadata']>) =>
    ...
    },
    {
      ...
      header: '銷售狀態',
      accessorKey: 'artworks.0.metadata',
      cell: ({ getValue }: CellContext<RepairReturnOrder, ArtworkDetail['metadata']>) => {
  ...
  const { table, tableBlock } = useTable<RepairReturnOrder>({
    data,
    columns,
    isLoading,
  });
