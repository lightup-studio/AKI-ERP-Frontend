...
import TransferOrderArtworkTable from './TransferOrderArtworkTable';

type FormData = {
  artworks: ArtworkDetail[];
};

interface TransferOrderBatchUpdateDialogProsp {
  list: Pick<TransferOrder, 'id' | 'artworks'>[];
  isOpen: boolean;
  onClose?: () => void;
}

const TransferOrderBatchUpdateDialog: React.FC<TransferOrderBatchUpdateDialogProsp> = ({
  list,
  isOpen = false,
  onClose,
}) => {
              ...
              .map((item) => (
                <div key={item.id}>
                  ...
                  <div className="overflow-x-auto">
                    <TransferOrderArtworkTable data={item.artworks} register={register} />
                  </div>
                </div>
              ))}
          </div>
...
export default TransferOrderBatchUpdateDialog;
