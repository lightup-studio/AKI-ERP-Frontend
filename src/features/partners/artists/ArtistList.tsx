...
import {
  ...
  deletePartnerList,
  fetchPartnerList,
} from 'data-access/apis/partners.api';
...
import UpdateArtistDialog from './ui/UpdateArtistDialog';
...
function ArtistList() {
  ...
  const createMutation = useMutation(
    (data: ArtistPartner) => createPartner(data),
    {
      onSuccess: () => {
  ...
  const deleteMutation = useMutation(
    (ids: number[]) => deletePartnerList(ids),
    {
      onSuccess: () => {
        if ([...searchParams.values()].length > 0) {
          setKeyword('');
  ...
  const [tableData, setTableData] = useState<ArtistPartner[]>([]);
  ...
  const handleAllRowSelectionChange = (rows: Row<ArtistPartner>[]) => {
  ...
  const handleRowSelectionChange = (row: Row<ArtistPartner>) => {
  ...
  const isLoading =
    dataQuery.isLoading || createMutation.isLoading || deleteMutation.isLoading;

  const columns: ColumnDef<ArtistPartner, any>[] = [
    {
      id: 'select',
      header: ({ table }) => (
        <div className="px-1 flex items-center">
          <IndeterminateCheckbox
            {...{
              ...
              indeterminate:
                selectedRowCount > 0 &&
                selectedRowCount < (dataQuery.data?.totalCount || 0),
              onChange: () =>
    ...
    },
    {
      ...
      cell: ({ cell, row }) => (
        ...
        >
          {cell.getValue()}{' '}
          <PencilSquareIcon className="h-4 w-4 inline-block" />
        </button>
      ),
    },
    {
    ...
    <>
      <div className="flex flex-col gap-4">
        <div className="card w-full p-6 bg-base-100 shadow-xl">
          <h2 className="text-xl font-bold border-l-8 border-accent pl-4 mb-6">
            新增藝術家
          </h2>
              ...
              <div className="relative flex-1">
                <input
                  className={classnames(
                    'input input-bordered w-32 text-center rounded-r-none',
                    {
                ...
                <input
                  className={classNames(
                    'input input-bordered w-56 text-center rounded-l-none',
                    {
